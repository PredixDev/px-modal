    ...
    <div class="modal" id="modal" on-transitionend="_handleTransitionEnd">
      <div id="box" class="modal__box shadow-modal" style="display:none">
        ...
        <div class="modal__triggers">
          ...
          <!-- Actions: Reject -->
          <template is="dom-if" if="{{!hideRejectButton}}">
          ...
          <!-- Actions: Accept -->
          <template is="dom-if" if="{{!hideAcceptButton}}">
...
`,
  ...
  properties: {
    ...
    /**
    * If the hide-accept-button property is set the default accept button will not be 
    ...
    */
    ...
    /**
    * If the hide-reject-button property is set the default reject button will not be 
    * be added to the dialog.        
    * */
    hideRejectButton: {
      type: Boolean,
  ...
  _handleTransitionEnd(evt) {
    if (dom(evt).rootTarget === this.$.modal) {
  ...
  findFocusableElements() {
    this._focusableElements = IronFocusablesHelper.getTabbableNodes(this);
    if (this._focusableElements.length) {
  ...
  _handleModalKeypress(evt) {
    if (evt.key === 'Tab' && !evt.shiftKey) {
       ...
       */
      const lastFocusedElement = dom(evt).rootTarget;
    ...
    if (evt.key === 'Tab' && evt.shiftKey) {
       ...
       */
      let lastFocusedElement = dom(evt).rootTarget;
      if (lastFocusedElement) {
        const lastFocusedElementIndex = this._focusableElements.indexOf(lastFocusedElement)
        if (this._focusableElements[lastFocusedElementIndex - 1]) {
          this._focusableElements[lastFocusedElementIndex - 1].focus()
  ...
  _handleRejectTriggerTapped(e) {
    ...
    const triggerContainer = dom(this.root).querySelector('#reject-trigger-container');
    /*
  ...
  _handleAcceptTriggerTapped(e) {
    const target = dom(e).rootTarget;
    const triggerContainer = dom(this.root).querySelector('#accept-trigger-container');
    /*
     * - First check: We should only close the modal if the user clicked on
     *   a button, not if this click event is sourced from the container.
     * - Second check: A click on a disabled input element like a <button>
